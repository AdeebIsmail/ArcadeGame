...
public class ShapeApplication extends Application {
    ...
    public void start(Stage stage) throws Exception {
        Skull = new Image("SImages/skull.png");
        ...
        canvas.setOnMousePressed(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent mouseEvent) {
                if (mouseEvent.getButton().name().equals("PRIMARY")) {
                    ...
                    if (Square.contains(x, y) && !gameOver) {
                        moveSquare();
                        points += 1;
                    }
                    if (Circle.contains(x, y) && !gameOver) {
                        moveCircle();
                        points += 3;
                    }
                    if (Triangle.contains(x, y) && !gameOver) {
                        moveTriangle();
                        points += 5;
                    }
                ...
                if (mouseEvent.getButton().name().equals("SECONDARY") && gameOver) {
                    reset();
                }
                paint(canvas.getGraphicsContext2D());
